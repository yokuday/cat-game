...
class MainUI:
    ...
    def exp_bar(self):
        ...
        max_width = w // 2.4
        draw_fitted_text(self.font, f"Lv. {self.info['current_level']}", text_x, text_y, max_width, text_size, BLACK, center_y=True)
        ...
        draw_fitted_text(self.font, exp_text, text_x, text_y, max_width, self.exp_bar_size[1], BLACK, align="right", center_y=True)
    ...
    def icons(self):
        ...
        if pr.check_collision_point_rec(mouse, rectangle):
            ...
            if pr.is_mouse_button_pressed(pr.MouseButton(0)):
                ...
                self.ui_window.prepare_to_send({"show_window": self.arrow_clicked})
        else:
